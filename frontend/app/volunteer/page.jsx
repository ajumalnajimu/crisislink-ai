...
export default function VolunteerPage() {
  ...
  useEffect(() => {
    ...
    if (savedId) {
      ...
      setRegistered(true);
      fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? 'http://' + window.location.hostname + ':5000' : 'http://localhost:5000')}/api/volunteer/${savedId}`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? 'http://' + window.location.hostname + ':5000' : 'http://localhost:5000')) + '/api/volunteer', {
        method: 'POST',
  ...
  const handleAccept = async () => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? 'http://' + window.location.hostname + ':5000' : 'http://localhost:5000')) + '/api/match/accept', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleCompleteMission = async () => {
    if (!matchId) return;
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || (typeof window !== 'undefined' ? 'http://' + window.location.hostname + ':5000' : 'http://localhost:5000')}/api/matches/${matchId}/complete`, {
        method: 'POST',
      });
      setMatchData(null);
      setMatchId(null);
